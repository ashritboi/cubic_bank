...
public class LoginController {
	...
	@GetMapping(value= {"/customer/forgotpassword"})
	public String forgotPassword(Model model) {
			return "customer/forgotPassword";	//accessDenied.html
	}
	
	@PostMapping(value= {"/customer/forgotpassword"})
	public String getUsername(@RequestParam String username, Model model) {
		...
		List<CustomerSaveAnswerVO>customerSaveAnswerVOs = (List<CustomerSaveAnswerVO>) securityQuestionService.findbyusername(username);
			if(customerSaveAnswerVOs.isEmpty()) {
				model.addAttribute("error","Sorry , please contact the help desk for login or check your email for password.");
				return "customer/forgotpassword";			
			}
		model.addAttribute("customerSecurityQueAnsVOs", customerSaveAnswerVOs);
		model.addAttribute("username", username);
		}
		else {
			model.addAttribute("error","Sorry , your username is not present in the database!");
			return "customer/forgotpassword";
		}
		return "customer/securityQuestionstest";
	}
	
	@PostMapping(value= {"/customer/checkans"})
	public String getAnswer(@RequestParam String username,@RequestParam String securityQuestionAnswer1, @RequestParam String securityQuestionAnswer2, Model model) {
		String nextPage = "customer/forgotPassword";
		List<CustomerSaveAnswerVO>customerSaveAnswerVOs = (List<CustomerSaveAnswerVO>) securityQuestionService.findbyusername(username);
		if(securityQuestionAnswer1.equalsIgnoreCase(customerSaveAnswerVOs.get(0).getAnswer())&& securityQuestionAnswer2.equalsIgnoreCase(customerSaveAnswerVOs.get(1).getAnswer())) {
			nextPage = "customer/resetPassword";
			model.addAttribute("username", username);
		}
		else {
			model.addAttribute("error","Sorry , your security answer donot match with our record.");
			model.addAttribute("customerSecurityQueAnsVOs", customerSaveAnswerVOs);
			model.addAttribute("username", username);
			return "customer/securityQuestionstest";
		}		
	...
	@PostMapping(value= {"/customer/resetnewPassword"})
	public String getnewPassword(@RequestParam String username,@RequestParam String password, @RequestParam String cpassword, Model model) {
		String nextPage = "customer/forgotPassword";
		ChangePasswordVO changePasswordVO = new ChangePasswordVO();
		changePasswordVO.setLoginid(username);
		changePasswordVO.setNewPassword(password);
		changePasswordVO.setConfirmPassword(cpassword);
		if(changePasswordVO.getNewPassword().equals(changePasswordVO.getConfirmPassword())) {
			nextPage ="customer/login";
			...
			model.addAttribute("username", username);
			return "customer/resetnewPassword";
		}
		return nextPage;
	}
